...
console.log('Starting remote Chrome...');
...
try {
  ...
  const browserPort = process.env.BROWSER_PORT_BACKUP
  ...
  const command = `/Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port=9222 --user-data-dir="${DIR}" --no-first-run --no-default-browser-check --disable-web-security --new-window http://localhost:${browserPort} &`;
  // when wanting a window without the address bar, i.e. app mode:
  // const command = `/Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port=9222 --user-data-dir="${DIR}" --no-first-run --no-default-browser-check --disable-web-security --app=http://localhost:${browserPort} &`
